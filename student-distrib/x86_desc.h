...
typedef union pde_4mb_t {
    ...
    struct {
        ...
        uint32_t pat               :1;
        uint32_t reserved          :9;
        uint32_t address           :10; /* all numbers refers to osdev */
    } __attribute__ ((packed));
} pde_4mb_t;
...
typedef union pde_4kb_t {
    ...
    struct {
        ...
        uint32_t available         :3;
        uint32_t address           :20; /* all numbers refers to osdev */
    } __attribute__ ((packed));
} pde_4kb_t;
...
/* Format the page directory entry and page table entry */
...
extern pte_t page_table_usermapping[PTE_TOT_NUM] __attribute__((aligned (FourKB)));
